		 ...
		 const char *reader_id, int do_wait, int verbose)
{
	...
	if (do_wait) {
		...
		if (sc_ctx_get_reader_count(ctx) == 0) {
			util_debug("Waiting for a reader to be attached...\n");
			r = sc_wait_for_event(ctx, SC_EVENT_READER_ATTACHED, &found, &event, -1, NULL);
			if (r < 0) {
				util_debug("Error while waiting for a reader: %s\n", sc_strerror(r));
				return 3;
			}
			r = sc_ctx_detect_readers(ctx);
			if (r < 0) {
				util_debug("Error while refreshing readers: %s\n", sc_strerror(r));
				return 3;
			}
		}
		util_debug("Waiting for a card to be inserted...\n");
		r = sc_wait_for_event(ctx, SC_EVENT_CARD_INSERTED, &found, &event, -1, NULL);
		if (r < 0) {
			util_debug("Error while waiting for a card: %s\n", sc_strerror(r));
			return 3;
		}
		reader = found;
	}
	else if (sc_ctx_get_reader_count(ctx) == 0) {
		util_debug("No smart card readers found.\n");
		return 1;
	}
	else   {
		if (!reader_id) {
			...
			for (i = 0; i < sc_ctx_get_reader_count(ctx); i++) {
				...
				if (sc_detect_card_presence(reader) & SC_READER_CARD_PRESENT) {
					util_debug("Using reader with a card: %s\n", reader->name);
					goto autofound;
				}
			}
			...
			if (is_string_valid_atr(reader_id))   {
				...
				for (i = 0; i < sc_ctx_get_reader_count(ctx); i++) {
					...
					util_debug("Matched ATR in reader: %s\n", rdr->name);
					reader = rdr;
					goto autofound;
				}
			}
...
autofound:
		if (!reader) {
			util_debug("Reader \"%s\" not found (%d reader(s) detected)\n",
					reader_id, sc_ctx_get_reader_count(ctx));
			return 1;
		}

		if (sc_detect_card_presence(reader) <= 0) {
			util_debug("Card not present.\n");
			return 3;
		}
	}

	if (verbose)
		util_debug("Connecting to card in reader %s...\n", reader->name);
	r = sc_connect_card(reader, &card);
	if (r < 0) {
		util_debug("Failed to connect to card: %s\n", sc_strerror(r));
		return 1;
	}

	if (verbose)
		util_debug("Using card driver %s.\n", card->driver->name);

	r = sc_lock(card);
	if (r < 0) {
		util_debug("Failed to lock card: %s\n", sc_strerror(r));
